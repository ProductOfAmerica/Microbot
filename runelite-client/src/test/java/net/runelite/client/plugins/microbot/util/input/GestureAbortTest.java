...
public class GestureAbortTest
{
	...
	public void oneGestureAtATimeAcrossThreads() throws Exception
	{
		...
		AtomicReference<Boolean> secondOverlapped = new AtomicReference<>(false);
		...
		Thread second = new Thread(() -> InputLoop.run(emit -> secondOverlapped.set(release.getCount() > 0)));
		second.start();
		Thread.sleep(120);

		assertFalse("second gesture must not run while the first holds the lock", secondOverlapped.get());
		release.countDown();
		first.join(2_000);
		second.join(2_000);
		assertFalse("second gesture ran only after the first finished", secondOverlapped.get());
	}
