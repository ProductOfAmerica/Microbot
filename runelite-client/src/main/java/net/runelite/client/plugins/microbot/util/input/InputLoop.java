...
/**
 * Runs one input gesture at a time and unwinds it if the human takes over partway through.
 *
 * <p>Serialising input across concurrent scripts is intentional: there is one cursor.
 *
 * <p>A lock, not a thread and queue. Script threads should block and the client thread already
 * defers through an executor, so a queue would buy nothing; a dedicated thread would also stall
 * every script whenever one gesture waited on a busy client thread.
 *
 * <p>The held-button set exists for drag. A triad is three events in microseconds and a
 * trajectory is a MOVED stream with nothing to unwind. Keys are not gesture-scoped and live in
 * {@code Rs2Keyboard}.
 */
