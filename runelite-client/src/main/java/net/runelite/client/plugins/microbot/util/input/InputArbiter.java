...
public final class InputArbiter
{
	...
	/**
	 * Distance is measured from the last position the bot wrote, not the previous real event:
	 * per-event deltas never accumulate, so twenty 3px moves would be 60px of travel and never
	 * cross the threshold.
	 *
	 * <p>Before the bot has emitted there is no reference and nothing to abort, so motion alone
	 * does not flip HUMAN. Buttons and keys still do.
	 */
	...
	/**
	 * Drops everything held, because the releases are no longer coming: a key held when the window
	 * deactivates never delivers its KEY_RELEASED. Since a held key suppresses idle resume, the
	 * stale entry would pin HUMAN forever. Ctrl held while taking a screenshot is the usual way in.
	 *
